...
const { recordOpen } = require('./analytics');
...
async function serveProject(req, res, { project, rawPath, store = projectStorage } = {}) {
  ...
  // Count an "open" for HTML page views only (not every image/CSS asset). The
  // slug is namespaced so project stats never collide with a single-file page
  // of the same name. Guarded internally — never blocks serving.
  if (/^text\/html/i.test(file.contentType || '')) {
    await recordOpen(req, res, {
      slug: `project:${project.slug}/${relPath}`,
