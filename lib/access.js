...
// Bulk ACL lookup for listings (/admin, the Telegram menu, list_pages).
//
// The per-slug getAcl above checks Blob AND Redis, which is right for serving
// one page but pathological for a list: 45 pages meant 45 concurrent Blob GETs,
// nearly all of them 404s for records that never left Redis. That is enough to
// blow a serverless function's time budget, and /admin awaits it before it will
// even show the dashboard.
//
// Instead: ONE Blob listing tells us which slugs actually have a Blob record.
// Only those get a Blob read; everything else goes straight to Redis.
//
// If the Blob listing fails the whole listing still renders from Redis alone —
// a degraded list beats a page that never loads.
async function getAclMap(slugs, deps = {}) {
  const blob = deps.blob || blobStorage;
  const readRedis = deps.kvGetJson || kvGetJson;
  const list = Array.isArray(slugs) ? slugs : [];

  let blobSlugs = new Set();
  if (blob.blobEnabled()) {
    try {
      blobSlugs = new Set(await blob.listAclSlugs());
    } catch (error) {
      console.error('[access] Blob ACL listing failed; falling back to Redis', error.message);
    }
  }

  const entries = await Promise.all(list.map(async (slug) => {
    const normalized = normalizeSlug(slug);
    if (blobSlugs.has(normalized)) {
      const fromBlob = await blob.readAcl(normalized).catch(() => null);
      if (fromBlob != null) return [slug, fromBlob];
    }
    const fromRedis = await readRedis(aclKey(normalized)).catch(() => null);
    return [slug, fromRedis || null];
  }));

  return Object.fromEntries(entries);
}

// Decides the access record a publish should write.
//
// Publishing new HTML must never change who can see a page. Callers that say
// nothing about access get the page's CURRENT record preserved; only a caller
// that explicitly asks for public (or passes an allow list) changes it. A page
// with no existing record is new, and new pages default to public.
//
// Without this, re-syncing or re-publishing a restricted page's HTML silently
// demoted it to public, because an absent `allow` was read as "allow nobody",
// which resolved to "not protected".
async function resolveAccessForPublish(slug, options = {}, deps = {}) {
  const { isProtected, allow = [], allowProvided = false } = options;
  const explicit = isProtected !== undefined || allowProvided;

  if (explicit) {
    return {
      protected: isProtected === undefined ? allow.length > 0 : Boolean(isProtected),
      allow
    };
  }

  const existing = await (deps.getAcl || getAcl)(slug, deps);
  if (existing && existing.protected) {
    return { protected: true, allow: (existing.allow || []).slice() };
  }
  return { protected: false, allow: [] };
}
