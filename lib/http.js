...
// bodies and cookies ourselves and resolve the WebAuthn Relying Party identity
// from the incoming request (with optional env overrides).
...
// WebAuthn requires a stable Relying Party ID (a domain) and an expected
// origin. Derive them from the request, allowing explicit env overrides for
// custom-domain or preview-deployment setups.
function resolveRelyingParty(req) {
  const host = requestHost(req);
  const hostname = host.split(':')[0];
  const proto = requestProto(req);

  const rpID = (process.env.WEBAUTHN_RP_ID || hostname || 'localhost').trim();
  const origin = (process.env.WEBAUTHN_ORIGIN || `${proto}://${host}`).trim();
  const rpName = (process.env.WEBAUTHN_RP_NAME || 'Lemzakov Data Room').trim();
