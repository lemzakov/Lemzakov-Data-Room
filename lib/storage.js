...
async function listSlugsByStore(prefix, deps = {}) {
  ...
  const [blobSlugs, redisSlugs] = await Promise.all([
    // A Blob outage must not take the whole listing down — /admin is the tool
    // used to dig out of a storage problem, so it has to render regardless.
    blob.blobEnabled()
      ? blob.listSlugs(prefix).catch((error) => {
          console.error('[storage] Blob slug listing failed; listing Redis only', error.message);
          return [];
        })
      : Promise.resolve([]),
    listRedis(prefix)
...
// Exposes the shared client to the maintenance CLI (scripts/drain-redis.js),
// which needs raw SCAN / DEL / MEMORY USAGE access. Application code should use
// the helpers above rather than reaching for the client directly.
async function getRedisClientForMaintenance() {
