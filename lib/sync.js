...
// Resolves how to authenticate to the Drive API: a service account (preferred,
// can read any folder shared with it) or a plain API key (public content only).
async function resolveAuth(config, options = {}) {
  if (config.serviceAccountJson) {
    const creds = parseServiceAccount(config.serviceAccountJson);
    const accessToken = await getAccessToken(creds, options);
    return { mode: 'serviceAccount', accessToken, clientEmail: creds.clientEmail };
  }
  return { mode: 'apiKey', apiKey: config.googleApiKey };
}
...
// empty list. They are harmless for ordinary "My Drive" folders. The API key is
// only appended in api-key mode; service-account mode authenticates via header.
function buildListUrl(folderId, auth) {
  const q = encodeURIComponent(`'${folderId}' in parents and trashed = false`);
  let url = `https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name,mimeType)&supportsAllDrives=true&includeItemsFromAllDrives=true`;
...
async function fetchFolderFiles(folderId, auth, options) {
  const list = await googleApiJson(buildListUrl(folderId, auth), { ...options, headers: authHeaders(auth) });
  return list.files || [];
}

async function fetchHtmlFile(fileId, auth, options) {
  const response = await fetchGoogleResponse(buildDownloadUrl(fileId, auth), {
    ...options,
...
async function diagnose(options = {}) {
  ...
  try {
    ...
    report.hint = 'Set GOOGLE_DRIVE_FOLDER_ID (or GOOGLE_DRIVE_FOLDER_LINK) and either GOOGLE_SERVICE_ACCOUNT_JSON (recommended) or GOOGLE_API_KEY in the Vercel project environment variables, then redeploy.';
    return report;
  }
  ...
  report.config = {
    ...
    serviceAccountConfigured: Boolean(config.serviceAccountJson)
  };

  let auth;
  try {
    auth = await resolveAuth(config, { fetchImpl, logger });
  } catch (error) {
    report.config.authMode = config.serviceAccountJson ? 'serviceAccount' : 'apiKey';
    report.summary = 'Authentication failed';
    report.hint = error.message;
    return report;
  }

  report.config.authMode = auth.mode;
  if (auth.mode === 'serviceAccount') {
    report.config.serviceAccountEmail = auth.clientEmail;
  }

  const listUrl = buildListUrl(config.folderId, auth);
  const safeUrl = sanitizeUrl(listUrl);
  logger.log('[diagnose] Google Drive list request', { url: safeUrl });

  let response;
  try {
    response = await fetchImpl(listUrl, { headers: authHeaders(auth) });
  } catch (error) {
  ...
  if (files.length === 0) {
    ...
    report.hint = auth.mode === 'serviceAccount'
      ? `Share the Drive folder with the service account email (${auth.clientEmail}) as Viewer, and double-check the folder ID. For a Shared Drive, add the service account as a member of the drive.`
      : 'An API key can only see publicly shared content. Share the folder AND its files as "Anyone with the link can view", double-check the folder ID, or switch to a service account (set GOOGLE_SERVICE_ACCOUNT_JSON).';
...
async function runSync(options = {}) {
  ...
  const auth = await resolveAuth(config, { fetchImpl, logger });
  ...
  const files = await fetchFolderFiles(config.folderId, auth, { fetchImpl, logger });
  const htmlFiles = files.filter(isHtmlFile);
  ...
  for (const file of htmlFiles) {
    ...
    try {
      const html = await fetchHtmlFile(file.id, auth, { fetchImpl, logger });
